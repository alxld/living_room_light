...
class LivingRoomLight(LightEntity):
    """Living Room Light."""

    def __init__(self) -> None:
        """Initialize Living Room Light."""
        self._light_lamps = light_entity_lamps
        self._light_ceiling = light_entity_ceiling
        self._light_all = light_entity_all
        self._name = "Living Room"
        # self._state = 'off'
        self._brightness = 0
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self._rightlight_lamps = RightLight(self._light_lamps, self.hass)
        self._rightlight_ceiling = RightLight(self._light_ceiling, self.hass)
        self._rightlight_all = RightLight(self._light_all, self.hass)
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        if rl:
            if self._brightness < 192:
                ...
                await self._rightlight_ceiling.disable_and_turn_off()
            else:
                await self._rightlight_lamps.disable()
                await self._rightlight_ceiling.disable()
                await self._rightlight_all.turn_on(
                    brightness=self._brightness,
                    brightness_override=self._brightness_override,
                )
        else:
            await self._rightlight_all.turn_on_specific(data)
    ...
    async def async_turn_on_mode(self, **kwargs: Any) -> None:
        ...
        self._brightness = 255
        self.switched_on = True
        await self._rightlight_all.turn_on(mode=self._mode)
        self.async_schedule_update_ha_state(force_refresh=True)
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        self._is_on = False
        self.switched_on = False
        await self._rightlight_all.disable_and_turn_off()
        await self._rightlight_lamps.disable_and_turn_off()
        await self._rightlight_ceiling.disable_and_turn_off()
